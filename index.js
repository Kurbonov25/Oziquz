...
bot.sendMessage(msg.chat.id,pict,{
   ...
   {
     var pict=`<b>Загрузите 1 фотографию для этого объявления!
...
❗️Если нет фото, пропустите этот шаг, нажав кнопку «Продолжить».</b>`;
bot.sendMessage(msg.chat.id,pict,{
    parse_mode:"HTML",
    reply_markup:{
...
`<a href="http://gkh-grodnoraion.by/wp-content/uploads/2018/03/%D0%BE%D0%B1%D1%8A%D1%8F%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5-1.jpg">&#160</a>`;
          ...
          }).then(function(resp)
          {
           ...
           ImageId_to_database=resp.message_id;
            ...
            db.query(`INSERT INTO sotish (user_id,category,location,phone_number,description,status,username,image_id) VALUES (${user_id},'${Category}','${Location}','${phoneNumber}','${description}',${status},'${username}',${ImageId_to_database})`);
          }) 
